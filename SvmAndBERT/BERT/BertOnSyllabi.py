...
os.chdir('..')
prevDirPath = os.getcwd()
# readData = os.path.join(prevDirPath, "inputTestSyllabi/" + file[:-4] + '.csv')
...
totalCt = 1
for filename in os.listdir(prevDirPath + '/SyllabiCsv'):
  tokenizer = BertTokenizer.from_pretrained(bert_model)
  device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
  print(filename)
  df = pd.read_csv(prevDirPath + '/SyllabiCsv/' + filename)
  result = []
  idees = []
  model = torch.load(currentDirPath + "/9.bin")
  ...
  print(totalCt)
  totalCt += 1
  with open(prevDirPath + '/SyllabiCsv/' + filename) as csvfile:
    datareader = csv.reader(x.replace('\0', '') for x in csvfile)
    ct = 0
    with open(prevDirPath + '/SyllabiResults/Bias' + filename[:-4] + '.txt', 'w') as out_file:
      out_file.write(" ")
    with open(prevDirPath + '/SyllabiResults/NotBias' + filename[:-4] + '.txt', 'w') as out_file:
      out_file.write(" ")

    next(datareader)
    for row in datareader:
      if(result[ct] == 1):
        with open(prevDirPath + '/SyllabiResults/Bias' + filename[:-4] + '.txt', 'a') as out_file:
          out_file.write(row[3] + '\n')
      else:
        with open(prevDirPath + '/SyllabiResults/NotBias' + filename[:-4] + '.txt', 'a') as out_file:
          out_file.write(row[3] + '\n')
      ct += 1
